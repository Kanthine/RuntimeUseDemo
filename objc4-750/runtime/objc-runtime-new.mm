                   ...
                   bool baseMethods, bool methodsFromBundle)
{
    ...
    bool scanForCustomAWZ = !cls->hasCustomAWZ();//类或父类实现默认的alloc/allocWithZone:方法
...
# endif
    ...
    for (EACH_HEADER) {
        ...
        for (i = 0; i < count; i++) {
            Class cls = (Class)classlist[i];// 数组中会取出OS_dispatch_queue_concurrent、OS_xpc_object、NSRunloop等系统类，例如CF、Fundation、libdispatch中的类。以及自己创建的类
    ...
    if (resolvedFutureClasses) {
        for (i = 0; i < resolvedFutureClassCount; i++) {
            realizeClass(resolvedFutureClasses[i]);// 实现懒加载的类
...
objc_copyClassList(unsigned int *outCount)
{
    ...
    realizeAllClasses();//获取 realized_class_hash 哈希表
...
IMP _category_getLoadMethod(Category cat)
{
    ...
    if (mlist) {
        ...
        for (const auto& meth : *mlist) {
            const char *name = sel_cname(meth.name);
            if (0 == strcmp(name, "load")) {//比较两个字符串:若相等，则返回零；
                return meth.imp;
            }
        }
    }
