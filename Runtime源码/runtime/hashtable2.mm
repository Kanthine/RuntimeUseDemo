...
/* 为了提高效率，bucket 包含一个指向数组的指针，或者在数组大小为1时直接指向数据 */
typedef union {
    const void    *one;
    const void    **many;
} oneOrMany;
...
unsigned NXCountHashTable (NXHashTable *table) {
    //直接返回 NXHashTable 结构体中的 count
    return table->count;
}

//返回一个布尔值：判断当前的 NXHashTable 中是否包含传入的数据
int NXHashMember (NXHashTable *table, const void *data) {
    ...
    unsigned    j = bucket->count;//在获取了 bucket 之后，根据其中元素个数的不同，选择不同的执行
    const void    **pairs;
...
//查看当前 data 是不是在表 table 中:如果在则返回表中的数据；如果不在则返回 NULL
void *NXHashGet (NXHashTable *table, const void *data) {
    ...
    if (! j) return NULL;//count == 0 返回 NULL
    if (j == 1) {
        //count == 1，使用 ISEQUAL 比较查找的数据与 bucket->elements.one 是否相等
        //如果相等，返回 bucket->elements.one ；如果不等，返回 NULL
        return ISEQUAL(table, data, bucket->elements.one)
...
void *NXHashInsert (NXHashTable *table, const void *data) {
    ...
    if (! j) {
        //count == 0 ，对应的 bucket 为空：将数据直接填入 bucket，增加 bucket 中元素的数目，以及 table 中存储的元素的数目：
        bucket->count++;
    ...
    if (j == 1) {
        ...
        *newt = data;
        ...
        table->count++;
        //在每次添加完一个元素之后，都会进行下面的判断：保证哈希表中的元素数据小于等于表中的 bucket 数量
        if (table->count > table->nbBuckets) _NXHashRehash (table);
        return NULL;
    };
    
    //如果原来的 bucket 中存储的元素大于 1，那么会在链表的头部追加一个新的元素：
    pairs = bucket->elements.many;
    while (j--) {
        /* we don't cache isEqual because lists are short */
...
/* 该函数会将 NXHashState 指向哈希表 table 的最末端：
 * 注意：最末端的位置其实并不属于 NXHashTable，它一定会为空。
 */
...
/* 该函数每调用一次，NXHashState 都会向前移动一次：
 * 如果已经移动到哈希表的最前端，则直接返回 NO，没有任何操作
 * 如果没有移动到哈希表的最前端：则将 NXHashState 向前移动一次，*data 指向哈希表中存储在该位置的数据，最后返回 YES
 */
int NXNextHashState (NXHashTable *table, NXHashState *state, void **data) {
    HashBucket *buckets = (HashBucket *) table->buckets;//哈希表中的元素
    
    while (state->j == 0) {
        if (state->i == 0)
            return NO;//移到最前端，返回 NO
        state->i--;
